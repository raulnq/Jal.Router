...
namespace Jal.Router.AzureServiceBus.Impl
{
    public class LoggerBusInterceptor : AbstractBusInterceptor
    {
        ...
        {
            _log.Info($"[Bus.cs, {method}, {options.Correlation}] Start Call. id: {context.Id} toconnectionstring: {context.ToConnectionString} topath: {context.ToPath} with origin: {context.Origin}");
        }
        ...
        {
            _log.Info($"[Bus.cs, {method}, {options.Correlation}] Message sent. id: {context.Id} toconnectionstring: {context.ToConnectionString} topath: {context.ToPath} with origin: {context.Origin}");
        }
