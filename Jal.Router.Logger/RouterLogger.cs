...
namespace Jal.Router.Logger
{
    public class RouterLogger : AbstractRouterLogger
    {
        ...
        {
            _log.Info($"[Router.cs, Route, {context.Id}] Start Call. Message arrived. id: {context.Id} from: {context.Origin.Name} origin: {context.Origin} retry: {context.RetryCount}");
        }

        public override void OnSuccess<TContent>(InboundMessageContext context, TContent content)
        {
            _log.Info( $"[Router.cs, Route, {context.Id}] Message routed. id: {context.Id} from: {context.Origin.Name} origin: {context.Origin} retry: {context.RetryCount}");
        }
