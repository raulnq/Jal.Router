...
On asynchronous scenarios is often needed to send a message (command) to some app and then wait for the result (event) of the process.
The following case will help us to understand how this library helps us to achieve this goal.

1. The App "A" sends a "Transfer" command to the App "B".
2. The App "B" routes the "Transfer" to the corresponding handler method.
3. The App "B" publishs a "Transferred" event with the result of the processing.
...
In order to send the message we need to implement a "Sender" class as follows.
```
public class Sender
{
...
```
The method Send receive two parameters, first the content of the message to be send and an instance of the class Options that has the following properties:
...
* ScheduledEnqueueDateTimeUtc, If we want to defer the delivery of the message to some time in the future.
...
```
When the message arrives the Route method dispatch it to the corresponding handler class.
```
...
public class TransferMessageHandler : IMessageHandler<Transfer>
{
    ...
    {
	//Do something
	_bus.Publish(transferred, new Origin{ Key = context.Origin }, new Options {Id = context.Id});
    }
}
```
The Publish method has the same parameters of the Send method the difference lies on the kind of message that is delivering. 
...
A new parameter is needed here: Origin, in order to mark this message as result message from the App "A". 
...
We start adding a "Listener" class for the result of the message.
...
public class Listener
{
    ...
    {
        _messageRouter_router.Route<Transferred>(message, "Source");
    }
}
```
...
public class RouterConfigurationSource : AbstractRouterConfigurationSource
{
    public RouterConfigurationSource()
    {
        ...
        RegisterRoute<ICompensableMessageHandler<Transferred>>("Source").ForMessage<Transferred>().ToBeHandledBy<TransferredSourceCompensableMessageHandler>(x =>
        {
            x.With<InboundMessageContext>(((transfer, handler, context) => handler.Handle(transfer, context))).When(((converted, handler) => handler.IsSuccessful(converted)));
            x.With<InboundMessageContext>(((transfer, handler, context) => handler.Compensate(transfer, context))).When(((converted, handler) => !handler.IsSuccessful(converted)));
        });
    }
}
```
